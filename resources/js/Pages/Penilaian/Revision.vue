          ...
          <form class="space-y-4" @submit.prevent="submit">
            ...
            >
              <div class="space-y-3">
                ...
                <div v-if="item.review_note" class="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-900">
                ...
                <div class="grid gap-3 lg:grid-cols-[1.05fr_0.95fr]">
                  <div class="rounded-xl border p-3">
                    ...
                    >
                      ...
                      <div class="space-y-1 p-4">
                        ...
                        <p class="text-xs text-muted-foreground">
                          {{ formatFileSize(item.original_file.size) }} • {{ formatDateTime(item.original_file.created_at) }}
                        </p>
                    ...
                    <div v-else class="mt-3 flex h-48 flex-col items-center justify-center rounded-2xl border border-dashed bg-slate-50 px-4 text-center text-sm text-muted-foreground">
                  ...
                  <div v-else class="rounded-xl border p-3">
                    ...
                    <div
                      ...
                      class="mt-3 rounded-xl border bg-slate-50 p-3"
                    >
                      ...
                      <p class="mt-1 text-xs text-muted-foreground">
                        {{ formatFileSize(item.replacement_file.size) }} - {{ formatDateTime(item.replacement_file.created_at) }}
                      </p>
                      <div class="mt-3">
                    ...
                    <div class="mt-2">
                      <ImageUpload
                        :model-value="form.replacements[item.id] ?? null"
                        :preview-kind="isImageItem(item) ? 'image' : 'document'"
                        :accept="item.accept"
                        :multiple="false"
                        :title="isImageItem(item) ? 'Upload foto pengganti' : 'Upload dokumen pengganti'"
                        :description="isImageItem(item)
    ...
    <Dialog :open="previewDialogOpen" @update:open="(open) => { if (!open) closePreview() }">
      <DialogContent class="max-h-[92vh] overflow-hidden p-0 sm:max-w-5xl">
        <DialogHeader class="border-b px-6 py-4">
          <DialogTitle class="truncate text-left">{{ currentPreviewFile?.original_name || currentPreviewItem?.target_label || 'Preview File' }}</DialogTitle>
        ...
        <div class="flex max-h-[calc(92vh-88px)] flex-col bg-muted/10">
          <div class="flex-1 overflow-auto p-4">
            <div
              class="flex min-h-[60vh] items-center justify-center overflow-hidden rounded-xl border bg-background"
            >
              <template v-if="previewMode === 'image' && currentPreviewFile?.url">
                <div class="relative flex h-[68vh] w-full items-center justify-center overflow-hidden bg-slate-100">
                  ...
                  <img
                    ...
                    :alt="currentPreviewFile.original_name"
                  ...
                  <div class="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-black/70 px-3 py-1 text-xs font-medium text-white">
              ...
              <div v-else-if="currentPreviewFile?.url" class="h-[68vh] w-full overflow-auto">
                <ReviewerFilePreview
                  ...
                  :name="currentPreviewFile.original_name"
                />
              </div>
            ...
            <div
              v-if="previewMode === 'image' && imagePreviewItems.length > 1"
              class="mt-4 space-y-3"
            >
              <div class="flex gap-3 overflow-x-auto pb-1">
                <button
                  v-for="(item, index) in imagePreviewItems"
                  :key="`revision-thumb-${item.id}`"
                  type="button"
            ...
            <div v-if="currentPreviewItem?.issue_note" class="mt-4 rounded-xl border bg-amber-50 px-4 py-3 text-sm text-amber-900">
