  ...
  <AdminLayout :title="resource.title">
    <div class="space-y-6">
      <section class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div><h1 class="text-3xl font-semibold tracking-tight text-slate-950">{{ resource.title }}</h1></div>
      ...
      <Card>
        <CardHeader><CardTitle>Filter</CardTitle><CardDescription>Kelola dokumen aktif dari admin Vue.</CardDescription></CardHeader>
        <CardContent class="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
          <div class="space-y-2"><Label for="q">Cari</Label><Input id="q" :model-value="filters.q" placeholder="Judul dokumen" @change="applyFilters({ q: $event.target.value })" /></div>
          <div class="space-y-2">
      ...
      <AdminCardList :items="records" empty-text="Belum ada dokumen." grid-class="grid gap-4 lg:grid-cols-2">
        <template #item="{ item }">
          <AdminEntityCard :title="item.title" :subtitle="item.company || '-'">
            <template #badges>
            ...
            <template #meta>
              ...
              <p>Efektif: {{ item.effective_since || '-' }}</p>
