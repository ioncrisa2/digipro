  ...
  <AdminLayout title="Detail Persetujuan Pengguna">
    <div class="mx-auto max-w-4xl space-y-6">
      <section class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div><h1 class="text-3xl font-semibold tracking-tight text-slate-950">{{ record.user_name }}</h1></div>
      ...
      <Card>
        ...
        <CardContent class="grid gap-4 md:grid-cols-2 text-sm text-slate-700">
          ...
          <div><p class="text-xs font-semibold uppercase tracking-widest text-slate-500">Dokumen</p><p class="mt-1">{{ record.document_title }}</p><p class="mt-1">{{ record.code }} Â· {{ record.version }}</p></div>
