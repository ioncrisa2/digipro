...
<template>
  <BlogPageLayout title="Artikel DigiPro by KJPP HJAR" description="Insight, panduan, dan pembacaan pasar properti dari DigiPro by KJPP HJAR.">
    <section class="border-b border-slate-200 bg-[var(--landing-navy)] text-white">
      <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
        <div class="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div class="max-w-3xl space-y-6">
            <p class="text-xs font-semibold uppercase text-[var(--landing-gold)]">
              DigiPro by KJPP HJAR
            </p>
            <h1 class="text-balance text-4xl font-bold leading-[1.02] sm:text-5xl lg:text-6xl">
              Perspektif appraisal untuk keputusan properti yang lebih tertata.
            </h1>
            <p class="max-w-2xl text-pretty text-base leading-7 text-slate-200 sm:text-lg">
              Catatan editorial DigiPro by KJPP HJAR membahas data pembanding, regulasi, risiko agunan, dan praktik kerja penilaian properti.
            </p>

            <div class="flex flex-col gap-3 sm:flex-row">
              <Link
                href="#article-feed"
                class="inline-flex items-center justify-center rounded-xl bg-[var(--landing-gold)] px-6 py-3 text-sm font-semibold text-[var(--landing-navy)] transition hover:bg-[#d5b66f] motion-reduce:transition-none"
              >
                Jelajahi Artikel
              </Link>
              <Link
                href="/"
                class="inline-flex items-center justify-center rounded-xl border border-white/20 px-6 py-3 text-sm font-semibold text-white transition hover:bg-white/10 motion-reduce:transition-none"
              >
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div class="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
              <p class="text-[11px] font-semibold uppercase text-[var(--landing-gold)]">Ruang baca</p>
              <p class="mt-3 text-3xl font-bold tabular-nums text-white">
                {{ rows.length }}
              </p>
            ...
            <div class="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
              <p class="text-[11px] font-semibold uppercase text-[var(--landing-gold)]">Fokus</p>
              <p class="mt-3 text-2xl font-bold text-white">
                Appraisal, risiko, dan regulasi
              </p>
              <p class="mt-2 text-pretty text-sm leading-6 text-slate-200">
    ...
    <section id="article-feed" class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
      <div class="grid gap-10 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div class="space-y-12">
          <section v-if="featuredArticle" class="space-y-5">
            <div class="flex items-end justify-between gap-4">
              <div>
                <p class="text-[11px] font-semibold uppercase text-[var(--landing-gold-dark)]">Pilihan editor</p>
              ...
              <Link
                ...
                class="group grid overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_14px_34px_rgba(15,23,42,0.07)] transition-colors duration-150 hover:border-[var(--landing-gold)]/55 motion-reduce:transition-none lg:grid-cols-[1.05fr_0.95fr]"
              >
                <ArticleCover
                  ...
                  fallback-class="flex h-full w-full items-center justify-center bg-[var(--landing-navy)] text-sm font-semibold uppercase text-white/80"
                />
              ...
              <div class="flex flex-col justify-between p-7 sm:p-9">
                <div class="space-y-5">
                  <ArticleMeta
                    ...
                    category-class="rounded-lg bg-[var(--landing-gold-soft)] px-3 py-1 text-[11px] font-semibold uppercase text-[var(--landing-gold-dark)]"
                  />

                  <div class="space-y-4">
                    <h3 class="text-balance text-3xl font-bold leading-tight text-[var(--landing-navy)] sm:text-4xl">
                ...
                <div class="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-[var(--landing-navy)]">
                  Baca artikel utama
          ...
          <section class="space-y-5">
            ...
            <div v-if="regularArticles.length" class="divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.045)]">
              <Link
                v-for="article in regularArticles"
                :key="article.slug"
                :href="route('articles.show', article.slug)"
                class="group grid gap-5 px-5 py-5 transition-colors first:rounded-t-2xl last:rounded-b-2xl hover:bg-slate-50/80 motion-reduce:transition-none sm:grid-cols-[220px_minmax(0,1fr)] sm:px-6"
              >
                <ArticleCover
                  :cover-path="article.cover_image_path"
                  :alt="article.title"
                  fallback-text="DigiPro by KJPP HJAR"
                  wrapper-class="aspect-[4/3] overflow-hidden rounded-xl bg-slate-200/70"
                  image-class="h-full w-full object-cover"
                ...
                <div class="min-w-0 py-1">
                  ...
                  <h3 class="mt-3 text-balance text-2xl font-bold leading-tight text-[var(--landing-navy)]">
                    {{ article.title }}
                  </h3>
                  ...
                  <div v-if="article.tags?.length" class="mt-4 flex flex-wrap gap-2">
                    <span
                      ...
                      :key="tag"
                      class="rounded-lg bg-slate-100 px-3 py-1 text-[11px] font-medium text-slate-600"
                    >
                      #{{ tag }}
                    </span>
                  </div>
            ...
            <div v-if="links.length > 3" class="flex flex-wrap items-center justify-center gap-2 pt-2">
              <Link
                ...
                :key="idx"
                :href="link.url || '#'"
                class="inline-flex min-h-11 min-w-[44px] items-center justify-center rounded-xl px-4 text-sm font-medium transition motion-reduce:transition-none"
                :class="
                  link.active
        ...
        <aside class="space-y-5 lg:sticky lg:top-28 lg:self-start">
          <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_28px_rgba(15,23,42,0.045)]">
            ...
            <form class="mt-4 space-y-4" @submit.prevent="submitSearch">
              <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 focus-within:ring-2 focus-within:ring-[var(--landing-navy)]/20 focus-within:ring-offset-2 focus-within:ring-offset-white">
              ...
              <div class="grid grid-cols-3 gap-2">
                <label v-for="option in searchScopeOptions" :key="option.value" class="cursor-pointer">
                  ...
                  <span
                    class="block rounded-lg px-3 py-2 text-center text-[11px] font-semibold uppercase transition motion-reduce:transition-none"
                    :class="
              ...
              <button
                type="submit"
                class="inline-flex w-full items-center justify-center rounded-xl bg-[var(--landing-navy)] px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"
              >
                Jalankan Pencarian
              </button>
            </form>
          ...
          <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_28px_rgba(15,23,42,0.045)]">
            ...
            <div class="mt-5 space-y-2">
              <Link
                ...
                class="flex items-center justify-between rounded-xl px-4 py-3 text-sm font-medium transition motion-reduce:transition-none"
                :class="!activeCategory ? 'bg-[var(--landing-navy)] text-white' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'"
              >
          ...
          <section v-if="tags.length" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_28px_rgba(15,23,42,0.045)]">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
              <Link
                ...
                class="rounded-lg border border-slate-200 bg-slate-100 px-3 py-2 text-[11px] font-semibold uppercase text-slate-700 transition hover:bg-slate-200/60 motion-reduce:transition-none"
              >
                #{{ tag.name }}
              </Link>
            </div>
