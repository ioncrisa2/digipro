...
class AdjustmentWorkbench extends Page
{
    protected static ?string $navigationIcon = 'heroicon-o-table-cells';

    protected static bool $shouldRegisterNavigation = false;

    protected static ?string $title = 'Adjustment Matrix';

    protected static string $view = 'filament.reviewer.pages.adjustment-workbench';
    ...
    {
        $this->assetId = (int) request()->integer('asset');
        ...
        $this->guidelineYear = $asset->request?->guidelineSet?->year
            ...
            : ($asset->request?->requested_at ? (int) $asset->request->requested_at->format('Y') : null);
        ...
        $this->contextMeta = [
            ...
            'guideline' => $asset->request?->guidelineSet?->name ?? '-',
    ...
    {
        $requestNumber = (string) ($this->subjectAsset['request_number'] ?? '');
        $suffix = $requestNumber !== '' ? " - {$requestNumber}" : '';

        return "Adjustment Matrix{$suffix}";
    }

    protected function getHeaderActions(): array
    {
        return [
            Action::make('saveAdjustments')
                ->label('Simpan Adjustment')
                ->icon('heroicon-o-check-circle')
                ->color('success')
                ->visible(fn (): bool => count($this->comparables) > 0)
                ->action('saveAdjustments'),

            Action::make('backToAsset')
                ->label('Kembali ke Aset')
                ->icon('heroicon-o-arrow-left')
                ->url(fn (): string => AppraisalAssetResource::getUrl('view', [
                    'record' => $this->assetId,
                ])),
            ...
            Action::make('openComparables')
                ...
                ->color('warning')
                ->url(fn (): string => AppraisalComparableResource::getUrl('index', [
    ...
    private function buildSubjectAsset(AppraisalAsset $asset): array
    {
        ...
        return [
            ...
            'peruntukan' => $this->text($asset->peruntukan),
            ...
            'frontage' => '-',
            'fronting_road' => '-',
            'town_planning' => $this->text($asset->peruntukan),
            'plot_ratio' => '-',
            'asking_price' => '-',
            'likely_sale' => '-',
            'assumed_discount' => '-',
    ...
    private function normalizeComparable(AppraisalAssetComparable $item, int $index): array
    {
        ...
        if ($floorCount === null && $this->toFloat($item->raw_building_area) > 0) {
            $floorCount = 1;
        }
        ...
        $buildingArea = $this->firstNumber([
            $item->raw_building_area,
        ...
        return [
            ...
            'building_area' => $this->formatArea($this->firstFilled([$item->raw_building_area, data_get($snapshot, 'luas_bangunan')], null)),
    ...
    private function recalculateAdjustmentOutputs(): void
    {
        ...
        foreach ($this->comparables as $comparable) {
            ...
            foreach ($factorKeys as $factorKey) {
                ...
                $this->adjustmentInputs[$comparableId][$factorKey] = $percent;
    ...
    public function updatedAdjustmentInputs(): void
    {
        $this->recalculateAdjustmentOutputs();
    }
    ...
    public function saveAdjustments(): void
    {
        ...
        if (count($this->comparables) === 0) {
            Notification::make()
                ->title('Belum ada pembanding dipilih')
                ->warning()
                ->send();
    ...
    private function calculateLandResidualContext(
        ...
        $residualLandValue = $buildingValue !== null
            ...
            : null;
        $residualLandValuePerSqm = ($residualLandValue !== null && $buildingArea > 0)
            ? ($residualLandValue / $buildingArea)
            : null;
